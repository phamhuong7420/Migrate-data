...
INSERT INTO "QLPN"."PHAN_LOAIS"
  ...
  /*+ USE_HASH(B C) */
   ...
   MPL.id MA_LOAI_PHAN_LOAI, -- LOAI_PHAN_LOAI_ID (Gán NULL vì mã cũ là VARCHAR2)
   
   MPLTL.ID MA_TU_LOAI, -- TU_LOAI_ID (Gán NULL)
   
   MPLDL.ID MA_DEN_LOAI, -- DEN_LOAI_ID (Gán NULL)
   ...
   LEFT JOIN QLPN.DM_MA_PHAN_LOAIES MPL  ON MPL.MPL_MA = A.MA_LOAI_PHAN_LOAI
   LEFT JOIN QLPN.DM_PHAN_LOAI_GIAM_GIUS MPLTL  ON MPLTL.PLGG_MA = A.MA_TU_LOAI
   LEFT JOIN QLPN.DM_PHAN_LOAI_GIAM_GIUS MPLDL  ON MPLDL.PLGG_MA = A.MA_DEN_LOAI;